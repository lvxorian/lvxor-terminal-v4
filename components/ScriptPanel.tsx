...
export default function ScriptPanel() {
  ...
  return (
    <div className="flex h-full flex-col rounded-2xl border border-slate-800/60 bg-slate-900/60 p-4 backdrop-blur-sm">
      {/* Header */}
      <div className="mb-3 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-sm font-semibold text-white">Call Script</h2>
            <p className="text-[11px] text-slate-500 leading-tight">Postup pro první kontakt</p>
          </div>
        </div>
        ...
        {!editing && (
          <button
            onClick={startEdit}
            className="flex h-7 w-7 items-center justify-center rounded-lg text-slate-500 transition-all duration-200 hover:bg-slate-700/60 hover:text-indigo-400"
      ...
      {/* Content */}
      <div className="flex-1 min-h-0 space-y-2 overflow-y-auto text-sm leading-relaxed text-slate-300">
        {editing
          ? draft.map((section, i) => (
              <div
                key={i}
              ...
              >
                <p className="mb-1.5 text-xs font-semibold tracking-wide text-indigo-400 uppercase">
                  {section.title}
                </p>
                <textarea
      ...
      {/* Edit actions */}
      {editing && (
        <div className="mt-3 flex items-center gap-2 border-t border-slate-700/30 pt-3 shrink-0">
          <button
            onClick={saveEdit}
